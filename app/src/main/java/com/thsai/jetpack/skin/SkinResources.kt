package com.thsai.jetpack.skin
...
class SkinResources(context: Context, private val originResources: Resources, skinPath: String) : Resources(originResources.assets, originResources.displayMetrics, originResources.configuration) {
    ...
    fun loadSkin(context: Context, skinPath: String) {
        if (File(skinPath).exists()) {
            ...
            val mInfo = mPm.getPackageArchiveInfo(skinPath, PackageManager.GET_ACTIVITIES)
            skinPkgName = mInfo.packageName
            var asset = AssetManager::class.java.newInstance()
            var method = asset.javaClass.getMethod("addAssetPath", String::class.java)
    ...
    private fun getTheSameResId(id: Int): Int {
        if (id == 0)
            return 0
        var resName = originResources.getResourceName(id).substringAfterLast(":")
        var skinResId = try {
    ...
    override fun getColorStateList(id: Int): ColorStateList {
        var skinResId = getTheSameResId(id)
